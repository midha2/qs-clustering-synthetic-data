...
def cluster_wise_mean_comparison(data, categories):
    clusters = sorted(data['Cluster'].unique())
    cluster_means = {cluster: data[data['Cluster'] == cluster][categories].mean() for cluster in clusters}
    ...
    for cat, diff in sorted_differences:
        ...
        if cumulative_difference / total_difference >= 0.9:
            break
    ...
    colorful = [Patch(color=cluster_colors[i]) for i, cluster in enumerate(clusters)]
    grays = [Patch(color=gray_shades[i]) for i, cluster in enumerate(clusters)]

    ax1.legend(title="Clusters")

    # ax1.legend(handles=colorful + grays,
    #         labels=[f'Cluster {cluster}' for i, cluster in enumerate(clusters)] + [f'Insig. Cat. Cluster {cluster}' for i, cluster in enumerate(clusters)], fontsize=10)
...
class FilterData:
    ...
    def make_plots(self):
        ...
        self.plots['mean_comp'], self.plots['opt_mean_comp'] = cluster_wise_mean_comparison(self.data, self.raw_columns)
